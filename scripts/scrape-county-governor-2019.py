#!/usr/bin/env python3
"""
Scrapes county-level 2019 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2019_Kentucky_gubernatorial_election)
...
Writes data-entry/county_governor_results_2019.csv with columns:
state,county_name,county_id,dem_2019,gop_2019,oth_2019,total_2019
...
Only KY and MS - LA held a runoff (jungle primary produced no outright winner, unlike
2023) but its Wikipedia page has no by-parish table for either round (only "Parishes
that flipped..." and "By congressional district" sections), same structural dead end as
LA's 2023 governor race and 2016 Senate race; needs a separate source, not this scraper.

Same parser as scripts/scrape-county-governor-2020.py / -2021.py / -2022.py / -2023.py /
-2024.py / -2025.py / scrape-county-senate-2024.py (all their wikitext-format bugfixes
inherited, including 2022's MD running-mate-name stripping, NY 3-tier fusion-voting
header handling, PA duplicate-subheader-cell correction, MA `{{election table}}`
template-opened-table support, and 2020's data-driven vote_first cross-check) - see
those files' docstrings and memory/project_county_election_scrape.md for the full list
of gotchas this parser already handles.

Run from project root: python3 scripts/scrape-county-governor-2019.py
"""
