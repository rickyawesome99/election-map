#!/usr/bin/env python3
"""
Scrapes county-level 2023 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2023_Kentucky_gubernatorial_election)
...
Writes data-entry/county_governor_results_2023.csv with columns:
state,county_name,county_id,dem_2023,gop_2023,oth_2023,total_2023
...
Only KY and MS - LA held its 2023 governor race as a first-round-outright jungle
primary win (Landry >50% in round one) and its Wikipedia page has no by-parish
breakdown at all (just a state-level Election box), same structural dead end as
LA's 2016 Senate race; needs a separate source, not this scraper.

Same parser as scripts/scrape-county-governor-2024.py / -2025.py /
scrape-county-senate-2024.py (all their wikitext-format bugfixes inherited), plus
one new fix this year:
- **KY 2023's header row-group is two lines that each open with a `!` marker but
  join their own cells with `||` instead of `!!`** (`!rowspan="2"...|County ||
  colspan="2"...|Andy Beshear || ...`) - the closest precedent is the 2020 Senate
  script's TN fix (documented in memory/project_county_election_scrape.md as
  "header line opens with `!` but joins cells with `||`"), but that fix was never
  backported into the 2024 Senate script this governor pipeline was copied from.
  Generalized here rather than re-special-casing per state: both the header-block
  loop and split_row_cells() now pick the separator by checking which one is
  actually present in the line's body (`"!!" if "!!" in body else "||"`) instead
  of assuming it from the leading marker character.

Run from project root: python3 scripts/scrape-county-governor-2023.py
"""
