#!/usr/bin/env python3
"""
Fetches complete, official county-level 2018 U.S. House results for Indiana from the
Indiana Secretary of State's own ENR portal - same source and reasoning as
fetch-in-sos-house-2020.py/fetch-in-sos-house-2022.py (see fetch-in-sos-house-2022.py's
docstring for the full reverse-engineering narrative). Tried proactively this year
(before even running the MEDSL fill script) since this portal has now beaten MEDSL for
IN two years running (2020, 2022).

**2018's site version uses a different OFFICECATEGORYID than 2020/2022**: this year's
`statewideElectionsC_A.json` groups offices under a `List`/`Heading`/`Items` structure
(not the flatter category list 2020/2022 use) and "US Representative" is ID **2167**
(not 1005 - that ID is 2020/2022-specific, not stable across all years). Same
`OffCatC_{id}_A.json` URL template and same Region/Race/Candidate JSON shape once you're
past that, though - confirmed by fetching and inspecting fresh rather than assuming.

No special-election region to exclude (only 9 plain-numbered regions in this year's
JSON, no "District CDx Special" entry).

Writes/merges into data-entry/county_house_results_2018.csv. Run from project root:
python3 scripts/fetch-in-sos-house-2018.py
"""
...
DATA_URL = "https://enr.indianavoters.in.gov/archive/2018General/data/OffCatC_2167_A.json"
