#!/usr/bin/env python3
"""
Scrapes county-level 2017 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2017_Virginia_gubernatorial_election)
...
Writes data-entry/county_governor_results_2017.csv with columns:
state,county_name,county_id,dem_2017,gop_2017,oth_2017,total_2017
...
wikitext-format bugfixes inherited, including 2022's MD running-mate-name stripping,
NY 3-tier fusion-voting header handling, PA duplicate-subheader-cell correction, MA
`{{election table}}` template-opened-table support, 2020's data-driven vote_first
cross-check, and 2016's OR-special-election lessons) - see those files' docstrings and
memory/project_county_election_scrape.md for the full list of gotchas this parser
already handles.

Run from project root: python3 scripts/scrape-county-governor-2017.py
"""
...
OUT_CSV = os.path.join(ROOT, "data-entry/county_governor_results_2017.csv")
