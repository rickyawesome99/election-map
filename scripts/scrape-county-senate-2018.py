#!/usr/bin/env python3
"""
Scrapes county-level 2018 Senate results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2018_United_States_Senate_election_in_Arizona)
and cross-validates each state's summed totals against the existing state-level
row in data-entry/senate_past_results.csv.

Writes data-entry/county_senate_results_2018.csv with columns:
...
Mirrors scrape-county-senate-2020.py's parsing logic exactly (same wikitext-format
gotchas and bugfixes apply - bold-name stripping, caption-row header detection,
diacritic/compound-name FIPS matching, legend-table skipping, mixed !!/|| separator
handling); only YEAR, STATE_NAMES, and TITLE_OVERRIDES differ (none needed so far - see
below). See [[project_county_election_scrape]] memory for the gotcha list.

MN and MS both elected two Senators in 2018 (a regular Class 1 seat plus a special
election to fill a vacancy in another class - MN: Klobuchar regular + Smith special;
MS: Wicker regular + Hyde-Smith/Espy special), same "two real seats, one state" shape as
GA 2020. load_senate_year() (inherited from 2020) already prefers the non-Special row per
state, matching senate_past_results.csv's convention of one row per state per year in
this pipeline's existing data model (which only has room for one Senate result per
state+year, not one per seat).

Run from project root: python3 scripts/scrape-county-senate-2018.py
"""
...
STATE_NAMES = {
    # CA excluded on purpose: 2018's race was Feinstein vs. de León, both Democrats (a
    # same-party top-two general) - doesn't fit this pipeline's dem/gop two-party model,
    # and the countySenateData schema/map coloring assume a real D-vs-R contest. The
    # parser can handle CA 2018's flat-header table format fine (see the CA fallback
    # below) if this is ever revisited, but the *data* itself isn't the right shape for
    # this feature, not a parsing limitation.
...
# pattern. Empty so far for 2018 - kept for parity with the 2020 script in case a state
# turns out to need one once run.
TITLE_OVERRIDES = {}
...
INDEPENDENT_CITY_OVERRIDES = {
    ...
    # 2018's MD page disambiguates with parens instead of a suffix word.
...
def parse_state(state_abbr: str, wikitext: str):
    ...
    # WV 2018 uses a 3-tier header: row-group 0 is a coarse "County | Candidate | Total"
    ...
    # CA 2018 (Feinstein vs. de León, both Democrats - a same-party top-two general) uses
    # a flat header with no colspan grouping at all: "County !! Feinstein # !! Feinstein %
    # !! de León # !! de León % !! Margin # !! Margin % !! Total" - every cell is its own
    # colspan=1 column, so the colspan>=2 classification above finds zero candidate
    # blocks. Fall back to pairing adjacent "X #"/"X %" cells into a candidate block
    # (mirrors the colspan-based candidate block's shape exactly, so everything
    # downstream - vote extraction, matching - works unchanged).
    if not candidates:
        flat_cells = []
        for line in rows[0]:
            body = line[1:]
...
def load_senate_year():
    ...
    # MN/MS 2018: regular + special) - prefer the non-Special one, same convention as
    # OK 2022 / NE 2024 / GA 2020. A state whose ONLY row that year is itself flagged
    # Special (e.g. AZ 2020) would still be included via that row, not dropped by a
    # blanket "type != Special" filter - not needed for 2018 (checked, every 2018 state
    # has a non-Special row) but kept for parity/safety.
