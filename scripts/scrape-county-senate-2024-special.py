#!/usr/bin/env python3
"""
Scrapes county-level 2024 Senate SPECIAL election results from Wikipedia's "By county"
tables - Nebraska's special seat (Ricketts vs. Preston Love Jr., filling Ben Sasse's
resignation), the companion race to NE's REGULAR 2024 Senate election (Fischer vs. Dan
Osborn, already scraped by scrape-county-senate-2024.py).

Writes data-entry/county_senate_special_results_2024.csv with the SAME column shape as
the regular file (state,county_name,county_id,dem_2024,gop_2024,oth_2024,total_2024) so
...
Copy of scrape-county-senate-2024.py's exact parsing logic (unchanged - same wikitext
...
Run from project root: python3 scripts/scrape-county-senate-2024-special.py
"""
...
TITLE_OVERRIDES = {
    "NE": "2024 United States Senate special election in Nebraska",
}
...
def load_senate_year():
    # Mirror image of the regular script's filter: keep only each state's Special row for
