#!/usr/bin/env python3
"""
Scrapes county-level 2021 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2021_New_Jersey_gubernatorial_election)
...
Writes data-entry/county_governor_results_2021.csv with columns:
state,county_name,county_id,dem_2021,gop_2021,oth_2021,total_2021
...
Same parser as scripts/scrape-county-governor-2022.py / -2023.py / -2024.py / -2025.py /
scrape-county-senate-2024.py (all their wikitext-format bugfixes inherited, including
2022's MD running-mate-name stripping, NY 3-tier fusion-voting header handling, PA
duplicate-subheader-cell correction, and MA `{{election table}}` template-opened-table
support) - see those files' docstrings and memory/project_county_election_scrape.md for
the full list of gotchas this parser already handles.

Run from project root: python3 scripts/scrape-county-governor-2021.py
"""
...
OUT_CSV = os.path.join(ROOT, "data-entry/county_governor_results_2021.csv")
