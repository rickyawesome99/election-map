#!/usr/bin/env python3
"""
Scrapes county-level 2022 Senate results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2022_United_States_Senate_election_in_Arizona)
...
Evan McMullin -> dem in Utah).

Writes data-entry/county_senate_results_2022.csv with columns:
...
Mirrors scripts/scrape-county-senate-2024.py's parsing logic exactly (same
wikitext-format gotchas apply); only YEAR, STATE_NAMES, and the output paths
differ. See [[project_county_election_scrape]] memory for the gotcha list.

Run from project root: python3 scripts/scrape-county-senate-2022.py
"""
...
# The 34 states with a regular-class Senate race in 2022 (per senate_past_results.csv).
# Oklahoma also had a concurrent special election that year (Horn vs. Mullin) - we only
# scrape the regular-election page here, matching load_senate_year()'s type != "Special" filter.
STATE_NAMES = {
    "AK": "Alaska", "AL": "Alabama", "AR": "Arkansas", "AZ": "Arizona",
    "CA": "California", "CO": "Colorado", "CT": "Connecticut", "FL": "Florida",
    "GA": "Georgia", "HI": "Hawaii", "IA": "Iowa", "ID": "Idaho",
    "IL": "Illinois", "IN": "Indiana", "KS": "Kansas", "KY": "Kentucky",
    "LA": "Louisiana", "MD": "Maryland", "MO": "Missouri", "NC": "North Carolina",
    "ND": "North Dakota", "NH": "New Hampshire", "NV": "Nevada", "NY": "New York",
    "OH": "Ohio", "OK": "Oklahoma", "OR": "Oregon", "PA": "Pennsylvania",
    "SC": "South Carolina", "SD": "South Dakota", "UT": "Utah", "VT": "Vermont",
...
def fetch(state_abbr: str) -> str:
    title = f"{YEAR}_United_States_Senate_election_in_{STATE_NAMES[state_abbr]}"
...
def load_senate_year():
    # Oklahoma had both a regular and a special Senate race in 2022, each its own row
    # here; we always scrape the regular-election Wikipedia page (see fetch()), so only
    # keep the non-special row per state.
    m = {}
...
def main():
    ...
    for abbr in states:
        ...
        # already encodes overrides like independents caucusing with a party - e.g. Utah's
        # Evan McMullin sits in the "dem" bucket as the unified anti-Lee candidate). Only
        ...
        # nickname) - and only for that still-unclaimed slot, so a party-label fallback
        # can't also grab a second candidate for an already-matched slot.
