#!/usr/bin/env python3
"""
Scrapes county-level 2018 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2018_Texas_gubernatorial_election)
...
Writes data-entry/county_governor_results_2018.csv with columns:
...
AK excluded (no counties, no by-borough table on any year's page - established gap).
NE excluded - its "By county" table exists structurally but every value cell is an
unfilled `[[Person]]` template placeholder with no real data anywhere in the page
(confirmed - not a scraper bug, a genuinely incomplete Wikipedia table).
...
bugfixes inherited, including 2022's MD running-mate-name stripping, NY 3-tier
fusion-voting header handling, PA duplicate-subheader-cell correction, MA
`{{election table}}` template-opened-table support, and 2020's data-driven vote_first
cross-check) - see those files' docstrings and memory/project_county_election_scrape.md
for the full list of gotchas this parser already handles.

Run from project root: python3 scripts/scrape-county-governor-2018.py
"""
