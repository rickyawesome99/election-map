...
//       npx tsx scripts/tplBacktest.ts --emit     (also write data/tplValidation.ts — the table on /methodology/state-tpl)
//
...
function runHoldout(): void {
  ...
  let pass = true;
  const emitted: { target: string; predictor: string; n: number; mae: number; r: number; shift: number }[] = [];
  for (const [tKey, tLabel] of targets) {
    ...
    for (const [pKey, pLabel] of predictors) {
      ...
      if (!ok) pass = false;
      emitted.push({ target: tLabel, predictor: pLabel, n: e.n, mae: +e.mae.toFixed(2), r: +e.r.toFixed(3), shift: +e.shift.toFixed(2) });
      console.log(
  ...
  if (process.argv.includes("--emit")) {
    const file = path.join(process.cwd(), "data", "tplValidation.ts");
    fs.writeFileSync(file, `// GENERATED by \`npx tsx scripts/tplBacktest.ts --emit\` — do not edit by hand.\n// The 2024 holdout behind the Validation table on /methodology/state-tpl: each predictor is built from\n// information through 2022 and scored on 2024 after removing a uniform national shift.\nexport const TPL_VALIDATION = ${JSON.stringify({ generatedAt: new Date().toISOString().slice(0, 10), pass, rows: emitted }, null, 2)} as const;\n`);
