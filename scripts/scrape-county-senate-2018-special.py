#!/usr/bin/env python3
"""
Scrapes county-level 2018 Senate SPECIAL election results from Wikipedia's "By county"
tables (MN: Smith vs. Housley, filling Al Franken's vacancy; MS: Hyde-Smith vs. Espy,
filling Thad Cochran's vacancy) - the companion race to each state's REGULAR 2018 Senate
election (scrape-county-senate-2018.py), which this project's existing pipeline already
scrapes separately.

Writes data-entry/county_senate_special_results_2018.csv with the SAME column shape as
the regular file (state,county_name,county_id,dem_2018,gop_2018,oth_2018,total_2018) so
...
Copy of scrape-county-senate-2018.py's exact parsing logic (unchanged - same wikitext
gotchas apply); only STATE_NAMES/TITLE_OVERRIDES, OUT_CSV, and load_senate_year()'s
type=="Special" filter (the mirror image of the regular script's filter) differ. See
[[project_county_election_scrape]] memory for the gotcha list.

Run from project root: python3 scripts/scrape-county-senate-2018-special.py
"""
...
# Special-election pages use their own distinct Wikipedia title, not the
# "{YEAR} United States Senate election in {State}" pattern the regular scripts use.
TITLE_OVERRIDES = {
    "MN": "2018 United States Senate special election in Minnesota",
    "MS": "2018 United States Senate special election in Mississippi",
}
...
def extract_table(wikitext: str) -> list[str]:
    ...
    # year scrape wants. Not expected to fire on these dedicated special-election pages,
    # but kept for parity/safety with the other scripts.
