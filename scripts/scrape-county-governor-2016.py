#!/usr/bin/env python3
"""
Scrapes county-level 2016 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2016_Washington_gubernatorial_election)
...
Writes data-entry/county_governor_results_2016.csv with columns:
...
wikitext-format bugfixes inherited) - see those files' docstrings and
memory/project_county_election_scrape.md for the full list of gotchas this parser
already handles.

Run from project root: python3 scripts/scrape-county-governor-2016.py
"""
...
# is what caused OR to initially look like a Wikipedia gap (it isn't).
TITLE_OVERRIDES = {
    "OR": f"{YEAR}_Oregon_gubernatorial_special_election",
}
...
def fetch(state_abbr: str) -> str:
    title = TITLE_OVERRIDES.get(state_abbr, f"{YEAR}_{STATE_NAMES[state_abbr]}_gubernatorial_election")
...
def parse_state(state_abbr: str, wikitext: str):
    ...
    if not candidates:
        # Some pages (e.g. MA 2018) use a completely flat header with no colspan
        # grouping at all ("County | Baker % | Baker # | Gonzalez % | Gonzalez # |
        # ... | Total #") - the colspan>=2 candidate classifier above finds nothing.
        # Fall back to scanning for adjacent "Name #"/"Name %" cell pairs and
        # synthesize candidate blocks from those instead.
        flat_cells = split_row_cells(rows[0])
        new_blocks = []
        i = 0
        while i < len(flat_cells):
            cell = flat_cells[i].strip()
            m = re.match(r"^(.*?)\s*(#|%)$", cell)
            m2 = re.match(r"^(.*?)\s*(#|%)$", flat_cells[i + 1].strip()) if i + 1 < len(flat_cells) else None
            if m and m2 and m2.group(1).strip() == m.group(1).strip() and m2.group(2) != m.group(2):
                name = m.group(1).strip()
                party = None
                if re.search(r"Democratic", name, re.I):
                    party = "D"
                elif re.search(r"Republican", name, re.I):
                    party = "R"
                new_blocks.append({"kind": "candidate", "name": name, "party": party,
                                    "span": 2, "vote_first": m.group(2) == "#"})
                i += 2
                continue
            if not new_blocks:
                new_blocks.append({"kind": "county"})
            elif re.match(r"^Total", cell, re.I):
                new_blocks.append({"kind": "total"})
            elif re.match(r"^Margin$", cell, re.I):
                new_blocks.append({"kind": "margin", "span": 2})
            else:
                new_blocks.append({"kind": "other"})
            i += 1
        blocks = new_blocks
        candidates = [b for b in blocks if b["kind"] == "candidate"]
...
def load_governor_year():
    # OR's 2016 race was itself a Special election (Brown filling Kitzhaber's
    # unexpired term) with no separate Regular row that year - a blanket "skip
    # anything flagged Special" filter would drop it entirely. Group by state first
    # and only fall back to a Special row when no Regular row exists for that state
    # (same fix as AZ 2020's Senate special-only year).
    by_state = {}
    ...
    for state, rows in by_state.items():
        regular = [r for r in rows if r["type"] != "Special"]
        m[state] = regular[0] if regular else rows[0]
    return m
...
def main():
    ...
    # Merge rather than overwrite - this state list can be a subset (e.g. re-running
    # just to add one state), and OUT_CSV may also hold rows other scripts wrote for
    # states not handled here (e.g. fetch-openelections-governor-2016.py's MO/MT/ND/NC).
