#!/usr/bin/env python3
"""
Scrapes county-level 2022 Senate SPECIAL election results from Wikipedia's "By county"
tables - Oklahoma's special seat (Mullin vs. Kendra Horn, filling Jim Inhofe's
resignation), the companion race to OK's REGULAR 2022 Senate election (Lankford vs.
Madison Horn, already scraped by scrape-county-senate-2022.py).

Writes data-entry/county_senate_special_results_2022.csv with the SAME column shape as
the regular file (state,county_name,county_id,dem_2022,gop_2022,oth_2022,total_2022) so
...
Copy of scrape-county-senate-2022.py's exact parsing logic (unchanged - same wikitext
...
Run from project root: python3 scripts/scrape-county-senate-2022-special.py
"""
...
TITLE_OVERRIDES = {
    "OK": "2022 United States Senate special election in Oklahoma",
}
