#!/usr/bin/env python3
"""
Scrapes county-level 2025 Governor results from Wikipedia's "By county" tables
(e.g. https://en.wikipedia.org/wiki/2025_New_Jersey_gubernatorial_election)
...
Writes data-entry/county_governor_results_2025.csv with columns:
state,county_name,county_id,dem_2025,gop_2025,oth_2025,total_2025
...
Same parser as scripts/scrape-county-senate-2024.py (all its wikitext-format
bugfixes inherited: colspan-based header classification, bold/italic vote-value
stripping, multiple "By county" heading disambiguation, diacritic/compound-name
FIPS matching, "sortbottom"-or-name-based totals-footer skipping, etc.) - see
that file's docstring and memory/project_county_election_scrape.md for the
full list of gotchas this parser already handles.

Run from project root: python3 scripts/scrape-county-governor-2025.py
"""
...
OUT_CSV = os.path.join(ROOT, "data-entry/county_governor_results_2025.csv")
