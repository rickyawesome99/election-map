...
//    average tracks the current race while still incorporating older data.
...
// 4. aggregate = Σ(weight_i × diff_i) / Σ(weight_i), where diff = rep - dem (R-positive).
...
export type WeightedGenericBallotPoll = GenericBallotPoll & {
  ageDays: number;
  weight: number;
};

export type GenericBallotAverage = {
  diff: number;
  dem: number;
  rep: number;
  polls: WeightedGenericBallotPoll[];
};

export function computeGenericBallotAverage(
  asOf: Date = new Date(),
  ...
  const weighted: WeightedGenericBallotPoll[] = deduped.map((poll) => {
  ...
  const weightedMean = (pick: (p: WeightedGenericBallotPoll) => number) =>
    totalWeight > 0 ? weighted.reduce((sum, p) => sum + p.weight * pick(p), 0) / totalWeight : 0;

  const dem = weightedMean((p) => p.dem);
  const rep = weightedMean((p) => p.rep);

  return {
    diff: parseFloat((rep - dem).toFixed(1)),
    dem: parseFloat(dem.toFixed(1)),
    rep: parseFloat(rep.toFixed(1)),
    polls: weighted,
  };
}
