...
export default function OH31MapLeaflet2016({ activeRace, darkMode, onReady, townshipFilter, raceLabel }: Props) {
  const [geoData, setGeoData] = useState<GeoData | null>(null);
  const t = darkMode ? DARK_THEME : LIGHT_THEME;

  useEffect(() => {
    fetch("/oh31_2016_precincts.geojson")
      .then((r) => r.json())
  ...
  const styleFeature: StyleFunction = (feature) => {
    const name = (feature?.properties?.NAME16 as string) ?? "";
    ...
    <>
      ...
      <div style={{ borderRadius: 12, overflow: "hidden", border: "1px solid var(--app-border)", height: 520, position: "relative", zIndex: 0 }}>
        ...
        >
          ...
          />
          <GeoJSON
            key={`2016-${activeRace}-${darkMode}-${townshipFilter}`}
            data={geoData}
            style={styleFeature}
