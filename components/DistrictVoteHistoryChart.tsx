...
type ChartPoint = {
  year: string;
  demMargin: number;
  // negated nationalDiff: positive = district more D than national
  natY: number | null;
  demPct: number;
  repPct: number;
};
...
function MarginTooltip({ active, payload, label, showNational }: {
  ...
  const val = showNational ? d.natY : d.demMargin;
  if (val == null) return null;
  const displayValue = showNational ? val : d.demMargin;
  const displayIsDem = displayValue >= 0;
  const nationalDiffLabel = val === 0
    ? "Even with nation"
    : `${Math.abs(val).toFixed(1)} points more ${val >= 0 ? "D" : "R"} than nation`;
  return (
    <div
    ...
    >
      ...
      <div className="font-bold font-mono text-sm" style={{ color: displayIsDem ? "var(--party-dem)" : "var(--party-rep)" }}>
...
function getChartPoints(
  ...
  if (tab === "house") {
    return (houseResults ?? [])
      ...
      .map(r => {
        const demMargin = parseFloat((r.demPct - r.repPct).toFixed(1));
...
function buildSegmentLines(points: ChartPoint[], dataKey: "demMargin" | "natY"): {
...
export default function DistrictVoteHistoryChart({
  ...
  const marginVals = chartPoints.map(p => p.demMargin);
  ...
  const activeDataKey = showNational ? "natY" : "demMargin";
          ...
          <LineChart data={chartData} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
            ...
            <Line
              ...
              dot={({ cx, cy, payload }: { cx?: number; cy?: number; payload: ChartPoint }) => {
                const val = showNational ? payload.natY : payload.demMargin;
